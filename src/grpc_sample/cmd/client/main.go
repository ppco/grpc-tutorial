...
func main() {
	...
	conn, err := grpc.Dial(
		address,
		grpc.WithUnaryInterceptor(myUnaryClientInteceptor1),   // unary用middleware
		grpc.WithStreamInterceptor(myStreamClientInteceptor1), // stream用middleware
...
func HelloBiStreams() {
	...
	for !(sendEnd && recvEnd) {
		...
		if !recvEnd {
			if header == nil {
				header, err = stream.Header() // 一度ServerからSetHeaderされると、次回からは即座にレスポンスされる
				if err != nil {
					fmt.Println(err)
				} else {
					fmt.Printf("header:%+v \n", header)
